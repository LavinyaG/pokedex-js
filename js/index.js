...
async function search() {
    ...
    if (search == "") initializePage();
        ...
        const pokemmon = await searchPokemon();
        div.innerHTML = ` <div class="col">
        <a href="detalhes.html?numero=${pokemmon.id
            }"class="text-decoration-none">
        <div class="card" style="backgroud-color:${getCor(pokemon.types[0].type.name)}">
        <div id="imgPoke" class="card-img-top">${carousel(pokemon.id.sprites)}
        </div>
        <div class="card-body text-white">
        <h5 class="card-text mb-1">Nº ${pokemon.id
                .toString()
