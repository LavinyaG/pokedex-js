...
async function drawPokemon(id) {
    ...
    document.querySelector("h1").innerHTML = `${pokemon.id.toString()
        ...
        document.getElementById("peso").innerHTML = `${pokemon.height / 10} kg`;
        ...
        pokemon.types.forEach((value, index) => {
            ...
            buttons.innerHTML += `<button class="btn btn-lg btn-${name} text-white">${name}</button>`;
             ...
             sons.innerHTML = `<apan class="fw-bold mb-0 me-2">Sons:</span>`;
             if (pokemon.cries.latest != null)
                sons.innerHTML += `<i class="bi bi-play-circle fs-1 me-3" onclick="document.getElementById('latest').play()"></i><audio controls id='latest' hidden>
                ...
                onclick="document.getElementById('legacy').paly()"></i><audio controls id='legacy' hidden>
...
async function getPokemonAnterior(numero) {
    ...
    ${pokemonAnterior.id 
        .toString()
        .padStart(3, "0")}<br>${capitalizeFirstLetter(pokemonAnterior.name)}
        </button>`;
    ...
    async function getPokemonProximo(numero) {
        ...
        ${pokemonProximo.id
            .toString()
            .padStart(3, "0")}<br>${capitalizeFirstLetter(pokemonProximo.name)}
            </button>`;
            else return `<span></span>`;
    }
